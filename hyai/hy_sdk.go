...
// Chat 发起一个 Chat
func (t *TencentHyChat) Chat(ctx context.Context, req Request) (<-chan Response, error) {
...
func (t *TencentHyChat) synchronize(httpResp *http.Response, res chan Response) (err error) {
	defer func() {
		httpResp.Body.Close()
		close(res)
	}()
...
func (t *TencentHyChat) stream(httpResp *http.Response, res chan Response) {
	defer func() {
		httpResp.Body.Close()
		close(res)
	}()
...
//curl -H "Content-type: application/json" -X POST -d  '{"history":[{"role":"user","content":"给我讲个冷笑话"},{"role":"assistant","content":"电脑为什么冷，因为它有window"}],"query":"不好笑"}' http://localhost:30030/hyai/query
